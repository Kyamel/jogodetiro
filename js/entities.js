...
function registrarAmeacaChefe(entidade, sourceId, danoGerado) {
  if (!entidade || !entidade.isBoss || !sourceId || sourceId === entidade.id || sourceId === 'gas')
    return;
...
function darDano(entidade, dano, x, y, isCritical, sourceId) {
  ...
  if (entidade.armor && entidade.armor > 0) {
    if (dFinal >= entidade.armor) {
      dFinal -= entidade.armor;
      entidade.armor = 0;
      entidade.hp -= dFinal;
    } else {
      entidade.armor -= dFinal;
      dFinal = 0;
    }
  } else {
    entidade.hp -= dFinal;
  ...
  if (entidade.id === 'player' || entidade.isBoss) atualizarUI();
  mostrarDanoText(
    x,
    y,
    isCritical ? (dFinal > 0 ? dFinal : 'BLOCKED') : dFinal > 0 ? dFinal : 'BLOCKED',
    isCritical
  );
}
function moverEntidade(entidade, movX, movY) {
  let deslocX = movX * frameScale;
  let deslocY = movY * frameScale;
  let nX = entidade.x + deslocX;
  let nY = entidade.y + deslocY;
  let canX = true;
  let canY = true;
  for (let w of walls) {
    if (checkCollision({x: nX, y: entidade.y, radius: entidade.radius}, w)) canX = false;
  }
  for (let w of walls) {
    if (checkCollision({x: entidade.x, y: nY, radius: entidade.radius}, w)) canY = false;
  }
  if (nX < entidade.radius || nX > arena.width - entidade.radius) canX = false;
  if (nY < entidade.radius || nY > arena.height - entidade.radius) canY = false;
...
function moverBotPara(bot, ponto, multiplicador = 1) {
  ...
  moverEntidade(
    ...
    Math.sin(ang) * bot.speed * multiplicador
  );
}
