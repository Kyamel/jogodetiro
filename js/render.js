...
function desenharRetanguloArredondadoEm(
  ctxAlvo,
  x,
  y,
  w,
...
function prepararCamadasEstaticasArena() {
  ...
  baseCtx.lineWidth = 1;
  baseCtx.beginPath();
  for (let i = 0; i <= arena.width; i += 50) {
    baseCtx.moveTo(i, 0);
    baseCtx.lineTo(i, arena.height);
  }
  for (let i = 0; i <= arena.height; i += 50) {
    baseCtx.moveTo(0, i);
    baseCtx.lineTo(arena.width, i);
  }
  baseCtx.stroke();
  baseCtx.strokeStyle = 'rgba(255,255,255,0.075)';
  baseCtx.lineWidth = 1.5;
  baseCtx.beginPath();
  for (let i = 0; i <= arena.width; i += 250) {
    baseCtx.moveTo(i, 0);
    baseCtx.lineTo(i, arena.height);
  }
  for (let i = 0; i <= arena.height; i += 250) {
    baseCtx.moveTo(0, i);
    baseCtx.lineTo(arena.width, i);
  }
  baseCtx.stroke();
  ...
  walls.forEach((w) => {
    ...
    desenharRetanguloArredondadoEm(
      wallCtx,
      w.x,
      w.y,
      w.width,
...
function desenharBarraVidaPersonagem(x, y, teamId, isBoss, hp, maxHp, color) {
  ...
  preencherRetanguloArredondado(
    x - 30,
    barY,
    60 * Math.max(0, hp / maxHp),
    6,
    4,
    isBoss ? '#ff4d6d' : '#36d8ff'
  );
}
