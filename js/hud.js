...
function desenharMira() { if (!player || player.hp <= 0 || gameState !== "PLAYING") return; ctx.save(); ctx.translate(mouse.x, mouse.y); ctx.strokeStyle = "rgba(54,216,255,0.92)"; ctx.lineWidth = 2; ctx.shadowColor = "rgba(54,216,255,0.65)"; ctx.shadowBlur = REDUZIR_EFEITOS_CANVAS ? 0 : 12; ctx.beginPath(); ctx.arc(0, 0, 10, 0, Math.PI * 2); ctx.moveTo(-18, 0); ctx.lineTo(-7, 0); ctx.moveTo(7, 0); ctx.lineTo(18, 0); ctx.moveTo(0, -18); ctx.lineTo(0, -7); ctx.moveTo(0, 7); ctx.lineTo(0, 18); ctx.stroke(); ctx.restore(); }
