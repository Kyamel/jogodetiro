...
function listarAlvosChefe(chefe) {
  ...
  bots.forEach((o) => {
    if (o !== chefe && o.hp > 0 && o.teamId !== chefe.teamId) alvos.push(o);
  });
  return alvos;
}
...
function pontuarAlvoChefe(chefe, alvo, now) {
  ...
  let score =
    PESO_AMEACA_CHEFE * ameaca +
    PESO_VULNERABILIDADE_CHEFE * vulnerabilidade -
    PESO_DISTANCIA_CHEFE * dist;
...
function selecionarAlvoChefe(chefe, now) {
  ...
  let pontuados = listarAlvosChefe(chefe)
    .map((alvo) => pontuarAlvoChefe(chefe, alvo, now))
  ...
  if (!atual) {
    ...
    melhor.alvo.id !== atual.alvo.id &&
    now - (chefe.lastTargetSwitch || 0) >= TEMPO_MINIMO_TROCA_CHEFE
  ) {
    // Threshold e tempo minimo evitam que o chefao troque de alvo indeciso a cada quadro.
    let superouLimiar =
      atual.score <= 0
        ? melhor.score > atual.score + 15
        : melhor.score > atual.score * LIMIAR_TROCA_CHEFE;
    if (superouLimiar) {
      chefe.targetId = melhor.alvo.id;
      chefe.targetScore = melhor.score;
      chefe.lastTargetSwitch = now;
...
function calcularPosicaoPredita(atirador, alvo, velocidadeProjetil) {
  ...
  return {
    x: Math.max(
      alvo.radius || 0,
      Math.min(arena.width - (alvo.radius || 0), alvo.x + (alvo.velX || 0) * tempoEstimado)
    ),
    y: Math.max(
      alvo.radius || 0,
      Math.min(arena.height - (alvo.radius || 0), alvo.y + (alvo.velY || 0) * tempoEstimado)
    )
  };
}
function executarAtaqueChefe(chefe, infoAlvo, now) {
  ...
  if (dist < 115 && now - (chefe.lastMelee || 0) > 1100) {
    ...
    [player, ...bots].forEach((o) => {
      if (
        o &&
        o.hp > 0 &&
        o.teamId !== chefe.teamId &&
        Math.hypot(o.x - chefe.x, o.y - chefe.y) < 110
      )
        darDano(o, 60, o.x, o.y, true, chefe.id);
    });
    chefe.lastMelee = now;
    return;
  }
...
function atualizarChefeFSM(chefe, now) {
  ...
  if (!alvo && chefe.estado !== ESTADO_CHEFE.BUSCA) {
    if (chefe.ultimaPosicaoConhecida && now - (chefe.ultimaPosicaoConhecida.t || 0) < 5000)
      alterarEstadoChefe(chefe, ESTADO_CHEFE.BUSCA, now);
    else alterarEstadoChefe(chefe, ESTADO_CHEFE.PATRULHA, now);
  }
  if (chefe.estado === ESTADO_CHEFE.PATRULHA) {
    if (alvo && (visivel || dist < 800 || info.score > 20))
      alterarEstadoChefe(chefe, ESTADO_CHEFE.PERSEGUICAO, now);
  ...
  if (chefe.estado === ESTADO_CHEFE.PERSEGUICAO) {
    if (!alvo) {
      alterarEstadoChefe(
        chefe,
        chefe.ultimaPosicaoConhecida ? ESTADO_CHEFE.BUSCA : ESTADO_CHEFE.PATRULHA,
        now
      );
      return;
    }
    if (!visivel && now - (chefe.lastTargetSeen || 0) > 650) {
      alterarEstadoChefe(chefe, ESTADO_CHEFE.BUSCA, now);
      return;
    }
    if (visivel && dist < 520) {
      alterarEstadoChefe(chefe, ESTADO_CHEFE.COMBATE, now);
      return;
    }
  ...
  if (chefe.estado === ESTADO_CHEFE.COMBATE) {
    ...
    if (dist < 125 || (vidaRatio < 0.3 && now - chefe.estadoDesde > 1400)) {
  ...
  if (chefe.estado === ESTADO_CHEFE.RECUO) {
    ...
    else if (dist > 300 && now - chefe.estadoDesde > 900)
      alterarEstadoChefe(chefe, ESTADO_CHEFE.COMBATE, now);
    return;
  }
  if (chefe.estado === ESTADO_CHEFE.BUSCA) {
    if (alvo && visivel) {
      alterarEstadoChefe(chefe, dist < 520 ? ESTADO_CHEFE.COMBATE : ESTADO_CHEFE.PERSEGUICAO, now);
      return;
    }
    ...
    if (destino) {
      ...
      if (
        Math.hypot(destino.x - chefe.x, destino.y - chefe.y) < 45 ||
        now - (chefe.buscaInicio || now) > 4200
      ) {
