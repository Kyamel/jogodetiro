function update(deltaMs = MS_POR_FRAME) {
    ...
    for(let i=particles.length-1; i>=0; i--) { particles[i].x += particles[i].vx * frameScale; particles[i].y += particles[i].vy * frameScale; particles[i].life -= 0.02 * frameScale; if(particles[i].life <= 0) particles.splice(i, 1); } 
    for(let i=damageTexts.length-1; i>=0; i--) { damageTexts[i].y -= 1 * frameScale; damageTexts[i].life -= 0.02 * frameScale; if(damageTexts[i].life <= 0) damageTexts.splice(i, 1); } 
    for(let i=slashes.length-1; i>=0; i--) { slashes[i].life -= 0.08 * frameScale; if(slashes[i].life <= 0) slashes.splice(i, 1); } 
    for(let i=explosions.length-1; i>=0; i--) { explosions[i].life -= 0.05 * frameScale; if(explosions[i].life <= 0) explosions.splice(i, 1); }
    ...
    if (player.hp > 0) {
        ...
        if (teclaAtiva(" ", "Space", 32) && now - player.lastJump > 2000) { player.isJumping = true; player.jumpTimer = now; player.lastJump = now; }
        ...
        let movX = 0, movY = 0; if (teclaAtiva("w", "KeyW", 87) || teclaAtiva("arrowup", "ArrowUp", 38)) movY -= 1; if (teclaAtiva("s", "KeyS", 83) || teclaAtiva("arrowdown", "ArrowDown", 40)) movY += 1; if (teclaAtiva("a", "KeyA", 65) || teclaAtiva("arrowleft", "ArrowLeft", 37)) movX -= 1; if (teclaAtiva("d", "KeyD", 68) || teclaAtiva("arrowright", "ArrowRight", 39)) movX += 1; if (movX !== 0 || movY !== 0) { let len = Math.hypot(movX, movY); movX = (movX / len) * curSpd; movY = (movY / len) * curSpd; }
    ...
    for (let i = bots.length - 1; i >= 0; i--) {
        ...
        if (bot.isBoss) {
            ...
            if (modoJogo !== 'boss') {
                let caixaM = null; let distC = Infinity;
                caixas.forEach(cx => { let d = Math.hypot(cx.x - bot.x, cx.y - bot.y); if (d < distC) { distC = d; caixaM = cx; } });
                ...
                let contextoBot = {
                    ...
                    cobertura: cobertura,
                    distCobertura: cobertura ? Math.hypot(cobertura.x - bot.x, cobertura.y - bot.y) : Infinity,
        ...
        if (Math.hypot(bot.x - oldX, bot.y - oldY) < bot.speed * 0.2 * frameScale) { bot.stuckCounter = (bot.stuckCounter || 0) + frameScale; if (bot.stuckCounter > 20) { bot.patrolPoint = getValidSpawn(false, false, bot.isBoss ? 60 : 20); if (bot.isBoss) { let alvoAtual = obterEntidadePorId(bot.targetId); if (alvoAtual) bot.ultimaPosicaoConhecida = { x: alvoAtual.x, y: alvoAtual.y, t: now }; alterarEstadoChefe(bot, ESTADO_CHEFE.BUSCA, now); } else { bot.ignoreTargetUntil = now + 1500; } bot.stuckCounter = 0; } } else { bot.stuckCounter = 0; }
        atualizarVelocidadeEntidade(bot);
    }
    for (let i = bullets.length - 1; i >= 0; i--) {
        let b = bullets[i]; b.x += Math.cos(b.angle) * b.speed * frameScale; b.y += Math.sin(b.angle) * b.speed * frameScale; if (b.x < -500 || b.x > 2000 || b.y < -500 || b.y > 2000) { bullets.splice(i, 1); continue; }
